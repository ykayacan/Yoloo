package com.yoloo.android.ui.widget.floatingactionmenu;
...
public class FabWithOptions extends LinearLayout {
  ...
  private void init(Context context) {
    ...
    mMainFab.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.fab_icon));
  ...
  private void setupMainFab(Context context, TypedArray a) {
    ...
    int color = a.getColor(R.styleable.OptionsFabLayout_color,
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
      ...
      if (Build.VERSION.SDK_INT >= 17) params.setMarginEnd(pxToDp(context, 4));
  ...
  private void setupMiniFabs(Context context, TypedArray a) {
    ...
    int miniFabsColor = a.getColor(R.styleable.OptionsFabLayout_options_color,
  ...
  private void setupMarginsForMiniFab(Context context, FabWithTitleView FabWithTitleView) {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
      MarginLayoutParams miniFabParams =
          (MarginLayoutParams) FabWithTitleView.getFab().getLayoutParams();
      miniFabParams.setMargins(0, 0, pxToDp(context, 12), 0);
      if (Build.VERSION.SDK_INT >= 17) miniFabParams.setMarginEnd(pxToDp(context, 12));
   ...
   */
  public void setMainFabOnClickListener(final OnClickListener listener,
      final OptionsFabLayout layout) {

    mOptionsFabLayout = layout;
    mOptionsFabLayout.mRootView.setOnClickListener(view -> closeOptionsMenu());

    mMainFab.setOnClickListener(view -> {
      mMainFab.setSelected(!mMainFab.isSelected());
  ...
  private void visibilitySetup(int visible) {
    ...
    if (visible == View.VISIBLE) {
      for (int i = 0; i < mFabWithTitles.size(); i++) {
        ...
        final int finalI = i;
        new Handler().postDelayed(() -> enlargeAnim(mFabWithTitles.get(finalI).getFab()), i * 15);
        if (mFabWithTitles.get(finalI).getTitleEnable()) {
          new Handler().postDelayed(() -> {
            ...
            mFabWithTitles
                .get(finalI)
                .getCard()
                .startAnimation(AnimationUtils.loadAnimation(context, R.anim.fade_and_translate));
