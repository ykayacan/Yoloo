...
public class CustomFloatingActionMenuBehaviour
    extends CoordinatorLayout.Behavior<FloatingActionsMenu> {
  ...
  public CustomFloatingActionMenuBehaviour() {
  }
  ...
  public boolean onNestedPreFling(CoordinatorLayout coordinatorLayout,
      final FloatingActionsMenu child, View target, float velocityX, float velocityY) {
    ...
    if (velocityY < 0 && isScaleX(firstFab, 0f)) {
      ...
      if (child.isExpanded()) {
        child.collapse();
        child.setOnFloatingActionsMenuUpdateListener(
            new FloatingActionsMenu.OnFloatingActionsMenuUpdateListener() {
              @Override
              public void onMenuExpanded() {
  ...
  @Override
  public void onDependentViewRemoved(CoordinatorLayout parent, FloatingActionsMenu child,
      View dependency) {
    if (dependency instanceof Snackbar.SnackbarLayout
        && ViewCompat.getTranslationY(child) != 0.0F) {
