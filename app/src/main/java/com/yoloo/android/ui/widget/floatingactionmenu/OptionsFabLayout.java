...
public class OptionsFabLayout extends RelativeLayout {
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    bgColor = a.getColor(R.styleable.OptionsFabLayout_background_color,
  ...
  public void setMiniFabSelectedListener(final OnMiniFabSelectedListener listener) {
    for (int miniFabIndex = 0; miniFabIndex < mFabWithOptions.getMiniFabs().size();
        miniFabIndex++) {
      ...
      mFabWithOptions
          ...
          .getFab()
          .setOnClickListener(
              view -> listener.onMiniFabSelected(mFabWithOptions.getMenuItem(finalMiniFabIndex)));
      mFabWithOptions
          .getMiniFabs()
          .get(miniFabIndex)
          .getCard()
          .setOnClickListener(
              v -> listener.onMiniFabSelected(mFabWithOptions.getMenuItem(finalMiniFabIndex)));
      mFabWithOptions.getMiniFabs().get(miniFabIndex).getCard().setOnTouchListener((v, event) -> {
