...
class FeedHomePresenter extends MvpPresenter<FeedHomeView> {
    ...
    Disposable d = Observable
        .zip(getMeObservable().doOnNext(accountRealm -> Timber.d("Me: %s", accountRealm)),
            getTrendingCategoriesObservable().doOnNext(
                listOptional -> Timber.d("Trending cats: %s", listOptional.get().size())),
            getTrendingBlogsObservable().doOnNext(
                listOptional -> Timber.d("Trending blogs: %s", listOptional.get().size())),
            ...
            getNewUsersObservable().doOnNext(
                listOptional -> Timber.d("New users: %s", listOptional.get().size())),
            Group.Of6::create)
        .doOnNext(group -> {
  ...
  private Observable<Optional<List<PostRealm>>> getTrendingBlogsObservable() {
    return isFirstLoad ? postRepository
        .listByTrendingBlogPosts(null, 4)
        .map(Response::getData)
        .map(Optional::of)
        .observeOn(AndroidSchedulers.mainThread(), true) : Observable.just(Optional.empty());
  }
  ...
  private Observable<Optional<List<GroupRealm>>> getTrendingCategoriesObservable() {
  ...
  private Observable<Optional<List<AccountRealm>>> getNewUsersObservable() {
    return shouldLoadNewcomersComponent() ? userRepository
        ...
        .map(Response::getData)
        .map(Optional::of)
        .observeOn(AndroidSchedulers.mainThread()) : Observable.just(Optional.empty());
  }
  ...
  private List<FeedItem> mapGroupsToFeedItems(
      Group.Of6<AccountRealm, Optional<List<GroupRealm>>, Optional<List<PostRealm>>, Optional<Object>, Response<List<PostRealm>>, Optional<List<AccountRealm>>> group) {
    ...
    if (group.third.isPresent()) {
      FeedItem item = new TrendingBlogsFeedItem(group.third.get());
      feedItems.add(item);
    }
