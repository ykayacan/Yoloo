...
public final class MentionHelper {
  ...
  public static List<String> getMentions(String content) {
    List<String> matches = Lists.newArrayListWithCapacity(2);
  ...
  @Nullable
  public static List<Key<Account>> mentionedAccountKeys(Optional<String> mentionIds) {
    if (mentionIds.isPresent()) {
      Set<String> ids = StringUtil.splitToSet(mentionIds.get(), ",");

      return Observable.fromIterable(ids)
          .map(Key::<Account>create)
          .toList()
          .blockingGet();
    }
